...
import { setupTestDB, tokenUtil } from '../utils';
...
describe('Auth routes', () => {
  ...
  describe('POST /v1/auth/refresh-tokens', () => {
    test('should return 200 and new auth tokens if refresh token is valid', async () => {
      ...
      expect(dbRefreshTokenDoc).toMatchObject({ type: ETokenType.REFRESH, user: userUtil.userOne._id, blacklisted: false });
...
describe('Auth middleware', () => {
  test('should call next with no errors if access token is valid', async () => {
    await userUtil.insertUsers([userUtil.userOne]);
    const req = httpMocks.createRequest({ headers: { Authorization: `Bearer ${tokenUtils.userOneAccessToken}` } });
    const next = jest.fn();

    await auth(EUserRole.USER)(req, httpMocks.createResponse(), next);

    expect(next).toHaveBeenCalledWith();
    expect(req.user._id).toEqual(userUtil.userOne._id);
  });
  ...
  test('should call next with unauthorized error if access token is not found in header', async () => {
    ...
    expect(next).toHaveBeenCalledWith(
      expect.objectContaining({ statusCode: httpStatus.UNAUTHORIZED, message: '⛔ Please authenticate first!' })
    );
  });
  ...
  test('should call next with unauthorized error if the token is not an access token', async () => {
    ...
    await auth(EUserRole.USER, EUserRole.ADMIN)(req, httpMocks.createResponse(), next);
  ...
  test('should call next with unauthorized error if access token is expired', async () => {
    await userUtil.insertUsers([userUtil.userOne]);
    const expires = moment().subtract(1, 'minutes');
    const accessToken = tokenService.generateToken(userUtil.userOne._id.toString(), expires, ETokenType.ACCESS);
  ...
  test('should call next with unauthorized error if user is not found', async () => {
    ...
    expect(next).toHaveBeenCalledWith(expect.any(ApiError));
    expect(next).toHaveBeenCalledWith(
      expect.objectContaining({ statusCode: httpStatus.UNAUTHORIZED, message: '⛔ You don\'t have access to this ressource!' })
    );
  });
  ...
  test('should call next with forbidden error if user does not have required rights and userId is not in params', async () => {
    ...
    expect(next).toHaveBeenCalledWith(expect.any(ApiError));
    expect(next).toHaveBeenCalledWith(expect.objectContaining({ statusCode: httpStatus.FORBIDDEN, message: '⛔ You don\'t have access to this ressource!' }));
  });

  test('should call next with no errors if user does not have required rights but userId is in params', async () => {
    await userUtil.insertUsers([userUtil.userOne]);
    const req = httpMocks.createRequest({
      headers: { Authorization: `Bearer ${tokenUtil.userOneAccessToken}` },
